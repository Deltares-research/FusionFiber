...
# Format: location_key -> [start_position, end_position, reverse_direction, channel_name]
LOCATIONS = {
    "a-ch1": [529.6, 549.6, False, "channel 1"],
    "b-ch1": [549.6, 569.6, True, "channel 1"],
    "c-ch1": [924.8, 944.8, False, "channel 1"],
    "d-ch1": [944.8, 964.8, True, "channel 1"],
    "a-ch2": [529.6, 549.6, True, "channel 2"],
    "b-ch2": [924.8, 944.8, False, "channel 2"],
    "c-ch2": [549.6, 569.6, True, "channel 2"],
    "d-ch2": [529.6, 549.6, False, "channel 2"],
    "e-ch3": [529.35, 549.35, False, "channel 3"],
    "f-ch3": [549.35, 569.35, True, "channel 3"],
    "g-ch3": [924.3, 944.3, False, "channel 3"],
    "h-ch3": [944.3, 964.3, True, "channel 3"],
    "e-ch4": [944.8, 964.8, True, "channel 4"],
    "f-ch4": [549.6, 569.6, True, "channel 4"],
    "g-ch4": [924.8, 944.8, False, "channel 4"],
    "h-ch4": [944.8, 964.8, True, "channel 4"],
}
...
def _interval_data(all_channel_data: dict, location_key: str) -> tuple[pd.DataFrame, str, float, float]:
    ...
    start_pos, end_pos, _reverse, channel_name = LOCATIONS[location_key]
...
def _selected_measurement_bounds() -> tuple[float, float]:
    selected_starts = [LOCATIONS[key][0] for key in PLOT_LOCATIONS]
    selected_ends = [LOCATIONS[key][1] for key in PLOT_LOCATIONS]
...
def main() -> None:
    ...
    if not pickle_file.exists():
        channel_names = sorted({values[3] for values in LOCATIONS.values()})
