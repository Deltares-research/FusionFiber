...
TITLE '{id}'     { the problem identification }
...
DEFINITIONS
    ...
    transfer('..\head_optie1_output\head_Darcy{darcy_flux}.dat', head)

{---------------------------------------------Properties tank and sediment-------------------------------------------------------------------------------}
{groundwater properties}
	rhow=999																							{Density groundwater} {kg/m3}
	cw=4186																							{Specific heat groundwater}{J/kg*K}
...
{Combined properties}
	cb=n*rhow*cw+(1-n)*rhos*cs														{Combined volumetric specific heat}{J/(m**3*K)}
	Kb={bulk_conductivity}																	{Bulk conductivity of saturated sediment}
...
{Switch heat cables ON (1) or OFF (0)}
	Heat1_core = {bool_heat1}												{Switch for heat input in core of heating cable 1}
	Heat2_core = {bool_heat2}												{Switch for heat input in core of heating cable 2}
	Heat3_core = {bool_heat3}												{Switch for heat input in core of heating cable 3}
	Heat4_core = {bool_heat4}												{Switch for heat input in core of heating cable 4}


{HEAT INPUT to Cables}
	Voltage={voltage}												{Voltage for every timestep, will be calculated to heat input}
...
{Heating wire}												{Felten}						{literatuur}								{copper:  }
	...
	L_heat_core= {length}							{20}							{}													{Length of heating cable [m}
{Cable protection}
...
{Calculation of  heating by the core of the heating cable}
	...
	!HK {Q_heating=((Voltage^2)/(R_heat_core*L_heat_core))/L_heat_core}								{Calculated heat input for the cable}
	!Q_heating=(1-exp(-t/10))*((Voltage^2)/(R_heat_core*L_heat_core))/L_heat_core								{Calculated heat input for the cable}
    
    Q_heating = (1 - exp(-t / t_transition)) *
    					(Voltage^2 / (R_heat_core * L_heat_core^2)) *
    					(1 / (1 + exp(min((t - t_cutoff) / t_transition, 690))))

	{Heat input for the core of the heating cable, only when switch is ON}
	Q_heating1 = IF Heat1_core=1 THEN Q_heating ELSE 0
	Q_heating2 = IF Heat2_core=1 THEN Q_heating ELSE 0
	Q_heating3 = IF Heat3_core=1 THEN Q_heating ELSE 0
	Q_heating4 = IF Heat4_core=1 THEN Q_heating ELSE 0
...
{Heating right low}
	...
	REGION 31 "Heat1 core"
		SOURCE = Q_heating1/A_heating, Kb = Kc_heat_core, rhos=rhoc_heat_core, Cs=Cc_heat_core, Kx=0.000000001, n=0 
...
{Heating top right}
	...
	REGION 33 "Heat2 core"
		SOURCE = Q_heating2/A_heating, Kb = Kc_heat_core, rhos=rhoc_heat_core, Cs=Cc_heat_core, Kx=0.000000001, n=0 
...
{Heating left above}
	...
	REGION 35 "Heat3 core"
		SOURCE = Q_heating3/A_heating, Kb = Kc_heat_core, rhos=rhoc_heat_core, Cs=Cc_heat_core, Kx=0.000000001, n=0 
...
{Heating bottom left}
	...
	REGION 37 "Heat4 core"
		SOURCE = Q_heating4/A_heating, Kb = Kc_heat_core, rhos=rhoc_heat_core, Cs=Cc_heat_core, Kx=0.000000001, n=0 
...
{A3}
	...
	history(temp) at (Locx_A3_DTS1,Locy_A3_DTS1) (Locx_A3_DTS2,Locy_A3_DTS2) (Locx_A3_DTS3,Locy_A3_DTS3) (Locx_A3_DTS4,Locy_A3_DTS4)  fixed range (24,60) as 'DTS' export format"#1#b#2#b#3#b#4#b#t" file='{id}_05_A3_DTS_v5.txt'
